#!/bin/bash
# SSH Key Rotation Script for Production Environment
# Run this monthly to rotate SSH keys

set -e

echo "🔄 Rotating SSH keys for Production environment"
...
TEMP_DIR=$(mktemp -d)
ssh-keygen -t ed25519 -C "cloudflare-access-production-$(date +%Y%m%d)" -f "$TEMP_DIR/id_ed25519" -N "" -q

echo "📋 Manual 1Password steps:"
echo "1. Create new 'SSH Key' item in Production vault"
echo "2. Title: SSH Key - ssh-ca-production-$(date +%Y%m%d)"
echo "3. Upload private key: $TEMP_DIR/id_ed25519"
echo "4. Add public key text: $(cat $TEMP_DIR/id_ed25519.pub)"
echo "5. Add tags: ssh, cloudflare, production, rotated"
echo ""

echo "📋 Cloudflare Zero Trust Dashboard steps:"
echo "1. Go to Access > Service Auth"
echo "2. Update the Production SSH certificate"
echo "3. Replace public key with: $(cat $TEMP_DIR/id_ed25519.pub)"
echo ""

echo "📋 Server update steps:"
echo "1. Update /etc/ssh/cloudflare-ca-production.pub on all servers"
